...
exports.updateBookingById = exports.getBookings = exports.createBooking = exports.autoReturnExpiredBookings = void 0;
...
const autoReturnExpiredBookings = async () => {
    await db_1.dbPool.query(`WITH expired AS (
    ...
    FROM expired e
    WHERE b.id = e.id`);
    await db_1.dbPool.query(`UPDATE vehicles
...
exports.autoReturnExpiredBookings = autoReturnExpiredBookings;
const createBooking = async (options) => {
    const { requester, payload } = options;
    const customerId = requester.role === "customer" ? requester.id : Number(payload.customer_id);
    ...
    const vehicleRes = await db_1.dbPool.query(`SELECT vehicle_name, daily_rent_price, availability_status
     FROM vehicles
    ...
    const bookingRes = await db_1.dbPool.query(`INSERT INTO bookings (customer_id, vehicle_id, rent_start_date, rent_end_date, total_price, status)
    ...
    await db_1.dbPool.query(`UPDATE vehicles SET availability_status = 'booked' WHERE id = $1`, [vehicleId]);
...
const getBookings = async (options) => {
    ...
    await (0, exports.autoReturnExpiredBookings)();
    if (requester.role === "admin") {
        const result = await db_1.dbPool.query(`SELECT
         b.id,
         b.customer_id,
         b.vehicle_id,
         b.rent_start_date,
    ...
    const result = await db_1.dbPool.query(`SELECT
     ...
     ORDER BY b.id ASC`, [requester.id]);
    return result.rows.map((r) => ({
        id: r.id,
        vehicle_id: r.vehicle_id,
        rent_start_date: r.rent_start_date,
...
const updateBookingById = async (options) => {
    ...
    if (requester.role === "customer") {
        ...
        const bookingRes = await db_1.dbPool.query(`SELECT id, customer_id, vehicle_id, rent_start_date, rent_end_date, total_price, status
       FROM bookings
       WHERE id = $1`, [bookingId]);
        ...
        const updatedRes = await db_1.dbPool.query(`UPDATE bookings
       SET status = 'cancelled'
       WHERE id = $1
       RETURNING id, customer_id, vehicle_id, rent_start_date, rent_end_date, total_price, status`, [bookingId]);
        await db_1.dbPool.query(`UPDATE vehicles SET availability_status = 'available' WHERE id = $1`, [booking.vehicle_id]);
