...
exports.deleteVehicleById = exports.updateVehicleById = exports.getVehicleById = exports.getAllVehicles = exports.createVehicle = void 0;
const db_1 = require("../../config/db");
const allowedTypes = ["car", "bike", "van", "SUV"];
const allowedAvailability = ["available", "booked"];
const assertValidVehicleType = (type) => {
    if (typeof type !== "string" || !allowedTypes.includes(type)) {
        const error = new Error(`Invalid vehicle type. Allowed types: ${allowedTypes.join(", ")}`);
        error.statusCode = 400;
        throw error;
    }
};
const assertValidAvailability = (status) => {
    if (typeof status !== "string" ||
        !allowedAvailability.includes(status)) {
        const error = new Error(`Invalid availability status. Allowed: ${allowedAvailability.join(", ")}`);
        error.statusCode = 400;
        throw error;
    }
};
const createVehicle = async (payload) => {
    ...
    const result = await db_1.dbPool.query(`INSERT INTO vehicles (vehicle_name, type, registration_number, daily_rent_price, availability_status)
     VALUES ($1, $2, $3, $4, $5)
...
const getAllVehicles = async () => {
    const result = await db_1.dbPool.query(`SELECT id, vehicle_name, type, registration_number, daily_rent_price, availability_status
     FROM vehicles
     ORDER BY id ASC`);
    return result.rows;
};
...
const updateVehicleById = async (vehicleId, payload) => {
    if (payload.type !== undefined)
        assertValidVehicleType(payload.type);
    ...
    if (payload.vehicle_name !== undefined)
        add("vehicle_name", payload.vehicle_name);
    if (payload.type !== undefined)
    ...
    const result = await db_1.dbPool.query(`UPDATE vehicles
     SET ${fields.join(", ")}
     WHERE id = $${idx}
     RETURNING id, vehicle_name, type, registration_number, daily_rent_price, availability_status`, values);
    if (result.rows.length === 0) {
        const error = new Error("Vehicle not found");
        error.statusCode = 404;
        throw error;
...
const deleteVehicleById = async (vehicleId) => {
    ...
    const result = await db_1.dbPool.query(`DELETE FROM vehicles WHERE id = $1`, [vehicleId]);
